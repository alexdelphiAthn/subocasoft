object dmPre: TdmPre
  OnCreate = DataModuleCreate
  OnDestroy = DataModuleDestroy
  Height = 369
  Width = 1088
  object dsLinFac: TDataSource
    DataSet = unqryLinFac
    Left = 128
    Top = 88
  end
  object dsFacPrint: TDataSource
    DataSet = unqryFacPrint
    Left = 208
    Top = 88
  end
  object dsLinFacPrint: TDataSource
    DataSet = unqryLinFacPrint
    Left = 288
    Top = 88
  end
  object dsSeries: TDataSource
    DataSet = unqrySeries
    Left = 688
    Top = 88
  end
  object fxdsPrintFac: TfrxDBDataset
    Description = 'Presupuestos'
    UserName = 'Facturas'
    CloseDataSource = False
    FieldAliases.Strings = (
      'NRO_PRESUPUESTO=NRO_PRESUPUESTO'
      'SERIE_PRESUPUESTO=SERIE_PRESUPUESTO'
      'CODIGO_CLIENTE_PRESUPUESTO=CODIGO_CLIENTE_PRESUPUESTO'
      'RAZONSOCIAL_CLIENTE_PRESUPUESTO=RAZONSOCIAL_CLIENTE_PRESUPUESTO'
      'NIF_CLIENTE_PRESUPUESTO=NIF_CLIENTE_PRESUPUESTO'
      'MOVIL_CLIENTE_PRESUPUESTO=MOVIL_CLIENTE_PRESUPUESTO'
      'EMAIL_CLIENTE_PRESUPUESTO=EMAIL_CLIENTE_PRESUPUESTO'
      'DIRECCION1_CLIENTE_PRESUPUESTO=DIRECCION1_CLIENTE_PRESUPUESTO'
      'DIRECCION2_CLIENTE_PRESUPUESTO=DIRECCION2_CLIENTE_PRESUPUESTO'
      'POBLACION_CLIENTE_PRESUPUESTO=POBLACION_CLIENTE_PRESUPUESTO'
      'PROVINCIA_CLIENTE_PRESUPUESTO=PROVINCIA_CLIENTE_PRESUPUESTO'
      'CPOSTAL_CLIENTE_PRESUPUESTO=CPOSTAL_CLIENTE_PRESUPUESTO'
      'PAIS_CLIENTE_PRESUPUESTO=PAIS_CLIENTE_PRESUPUESTO'
      'FECHA_PRESUPUESTO=FECHA_PRESUPUESTO'
      'TOTAL_LIQUIDO_PRESUPUESTO=TOTAL_LIQUIDO_PRESUPUESTO'
      'FORMA_PAGO_PRESUPUESTO=FORMA_PAGO_PRESUPUESTO'
      'COMENTARIOS_PRESUPUESTO=COMENTARIOS_PRESUPUESTO'
      'NOMBRE=NOMBRE'
      'APELLIDOS=APELLIDOS'
      'DIBUJO_PRESUPUESTO=DIBUJO_PRESUPUESTO')
    DataSource = dsFacPrint
    BCDToCurrency = False
    DataSetOptions = []
    Left = 208
    Top = 24
  end
  object fxdstPrintLinFac: TfrxDBDataset
    Description = 'Lineas Presupuestos'
    UserName = 'Lineas Facturas'
    CloseDataSource = False
    FieldAliases.Strings = (
      'SERIE_PRESUPUESTO_LINEA=SERIE_PRESUPUESTO_LINEA'
      'NRO_PRESUPUESTO_LINEA=NRO_PRESUPUESTO_LINEA'
      'LINEA_LINEA=LINEA_LINEA'
      'CODIGO_ARTICULO_LINEA=CODIGO_ARTICULO_LINEA'
      'DESCRIPCION_ARTICULO_LINEA=DESCRIPCION_ARTICULO_LINEA'
      'ZONA=ZONA'
      'PRECIOVENTA_ARTICULO_LINEA=PRECIOVENTA_ARTICULO_LINEA'
      'CANTIDAD_LINEA=CANTIDAD_LINEA'
      'SUM_TOTAL_LINEA=SUM_TOTAL_LINEA'
      'ODONTOLOGO=ODONTOLOGO')
    DataSource = dsLinFacPrint
    BCDToCurrency = False
    DataSetOptions = []
    Left = 288
    Top = 24
  end
  object unqryFac: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO suboc_presupuestos'
      
        '  (NRO_PRESUPUESTO, SERIE_PRESUPUESTO, CODIGO_CLIENTE_PRESUPUESTO, RAZONSOCI' +
        'AL_CLIENTE_FACTURA, NIF_CLIENTE_PRESUPUESTO, MOVIL_CLIENTE_PRESUPUESTO, ' +
        'EMAIL_CLIENTE_PRESUPUESTO, DIRECCION1_CLIENTE_PRESUPUESTO, DIRECCION2_CL' +
        'IENTE_FACTURA, POBLACION_CLIENTE_PRESUPUESTO, PROVINCIA_CLIENTE_FACT' +
        'URA, CPOSTAL_CLIENTE_PRESUPUESTO, PAIS_CLIENTE_PRESUPUESTO, TIPOID_INT_C' +
        'LIENTE_FACTURA, FECHA_PRESUPUESTO, TOTAL_LIQUIDO_PRESUPUESTO, FORMA_PAGO' +
        '_FACTURA, COMENTARIOS_PRESUPUESTO, NOMBRE, APELLIDOS)'
      'VALUES'
      
        '  (:NRO_PRESUPUESTO, :SERIE_PRESUPUESTO, :CODIGO_CLIENTE_PRESUPUESTO, :RAZON' +
        'SOCIAL_CLIENTE_FACTURA, :NIF_CLIENTE_PRESUPUESTO, :MOVIL_CLIENTE_FAC' +
        'TURA, :EMAIL_CLIENTE_PRESUPUESTO, :DIRECCION1_CLIENTE_PRESUPUESTO, :DIRE' +
        'CCION2_CLIENTE_FACTURA, :POBLACION_CLIENTE_PRESUPUESTO, :PROVINCIA_C' +
        'LIENTE_FACTURA, :CPOSTAL_CLIENTE_PRESUPUESTO, :PAIS_CLIENTE_PRESUPUESTO,' +
        ' :TIPOID_INT_CLIENTE_PRESUPUESTO, :FECHA_PRESUPUESTO, :TOTAL_LIQUIDO_FAC' +
        'TURA, :FORMA_PAGO_PRESUPUESTO, :COMENTARIOS_PRESUPUESTO, :NOMBRE, :APELL' +
        'IDOS)')
    SQLDelete.Strings = (
      'DELETE FROM suboc_presupuestos'
      'WHERE'
      
        '  NRO_PRESUPUESTO = :Old_NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :Old_SERIE_' +
        'FACTURA')
    SQLUpdate.Strings = (
      'UPDATE suboc_presupuestos'
      'SET'
      
        '  NRO_PRESUPUESTO = :NRO_PRESUPUESTO, SERIE_PRESUPUESTO = :SERIE_PRESUPUESTO, CO' +
        'DIGO_CLIENTE_FACTURA = :CODIGO_CLIENTE_PRESUPUESTO, RAZONSOCIAL_CLIE' +
        'NTE_FACTURA = :RAZONSOCIAL_CLIENTE_PRESUPUESTO, NIF_CLIENTE_PRESUPUESTO ' +
        '= :NIF_CLIENTE_PRESUPUESTO, MOVIL_CLIENTE_PRESUPUESTO = :MOVIL_CLIENTE_F' +
        'ACTURA, EMAIL_CLIENTE_PRESUPUESTO = :EMAIL_CLIENTE_PRESUPUESTO, DIRECCIO' +
        'N1_CLIENTE_FACTURA = :DIRECCION1_CLIENTE_PRESUPUESTO, DIRECCION2_CLI' +
        'ENTE_FACTURA = :DIRECCION2_CLIENTE_PRESUPUESTO, POBLACION_CLIENTE_FA' +
        'CTURA = :POBLACION_CLIENTE_PRESUPUESTO, PROVINCIA_CLIENTE_PRESUPUESTO = ' +
        ':PROVINCIA_CLIENTE_PRESUPUESTO, CPOSTAL_CLIENTE_PRESUPUESTO = :CPOSTAL_C' +
        'LIENTE_FACTURA, PAIS_CLIENTE_PRESUPUESTO = :PAIS_CLIENTE_PRESUPUESTO, TI' +
        'POID_INT_CLIENTE_FACTURA = :TIPOID_INT_CLIENTE_PRESUPUESTO, FECHA_FA' +
        'CTURA = :FECHA_PRESUPUESTO, TOTAL_LIQUIDO_PRESUPUESTO = :TOTAL_LIQUIDO_F' +
        'ACTURA, FORMA_PAGO_PRESUPUESTO = :FORMA_PAGO_PRESUPUESTO, COMENTARIOS_FA' +
        'CTURA = :COMENTARIOS_PRESUPUESTO, NOMBRE = :NOMBRE, APELLIDOS = :APE' +
        'LLIDOS'
      'WHERE'
      
        '  NRO_PRESUPUESTO = :Old_NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :Old_SERIE_' +
        'FACTURA')
    SQLLock.Strings = (
      
        'SELECT NRO_PRESUPUESTO, SERIE_PRESUPUESTO, CODIGO_CLIENTE_PRESUPUESTO, RAZON' +
        'SOCIAL_CLIENTE_FACTURA, NIF_CLIENTE_PRESUPUESTO, MOVIL_CLIENTE_FACTU' +
        'RA, EMAIL_CLIENTE_PRESUPUESTO, DIRECCION1_CLIENTE_PRESUPUESTO, DIRECCION' +
        '2_CLIENTE_FACTURA, POBLACION_CLIENTE_PRESUPUESTO, PROVINCIA_CLIENTE_' +
        'FACTURA, CPOSTAL_CLIENTE_PRESUPUESTO, PAIS_CLIENTE_PRESUPUESTO, TIPOID_I' +
        'NT_CLIENTE_FACTURA, FECHA_PRESUPUESTO, TOTAL_LIQUIDO_PRESUPUESTO, FORMA_' +
        'PAGO_FACTURA, COMENTARIOS_PRESUPUESTO, NOMBRE, APELLIDOS FROM suboc_' +
        'presupuestos'
      'WHERE'
      
        '  NRO_PRESUPUESTO = :Old_NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :Old_SERIE_' +
        'FACTURA'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT NRO_PRESUPUESTO, SERIE_PRESUPUESTO, CODIGO_CLIENTE_PRESUPUESTO, RAZON' +
        'SOCIAL_CLIENTE_FACTURA, NIF_CLIENTE_PRESUPUESTO, MOVIL_CLIENTE_FACTU' +
        'RA, EMAIL_CLIENTE_PRESUPUESTO, DIRECCION1_CLIENTE_PRESUPUESTO, DIRECCION' +
        '2_CLIENTE_FACTURA, POBLACION_CLIENTE_PRESUPUESTO, PROVINCIA_CLIENTE_' +
        'FACTURA, CPOSTAL_CLIENTE_PRESUPUESTO, PAIS_CLIENTE_PRESUPUESTO, TIPOID_I' +
        'NT_CLIENTE_FACTURA, FECHA_PRESUPUESTO, TOTAL_LIQUIDO_PRESUPUESTO, FORMA_' +
        'PAGO_FACTURA, COMENTARIOS_PRESUPUESTO, NOMBRE, APELLIDOS FROM suboc_' +
        'presupuestos'
      'WHERE'
      '  NRO_PRESUPUESTO = :NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :SERIE_PRESUPUESTO')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_presupuestos')
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT F.*'
      'from suboc_presupuestos F'
      'order by serie_factura desc, F.NRO_PRESUPUESTO desc')
    AfterInsert = zqryFacAfterInsert
    BeforePost = zqryFacBeforePost
    AfterPost = unqryFacAfterPost
    BeforeDelete = zqryFacBeforeDelete
    AfterDelete = zqryFacAfterDelete
    Left = 48
    Top = 146
  end
  object unqryFacPrint: TUniQuery
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT * '
      'from v_suboc_presupuestos'
      'order by NRO_PRESUPUESTO Asc')
    Left = 208
    Top = 144
  end
  object unqryLinFacPrint: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO `SUBOC_FACTURAS_LINEAS`'
      
        '  (`SERIE_PRESUPUESTO_LINEA`, `NRO_PRESUPUESTO_LINEA`, `LINEA_LINEA`, `C' +
        'ODIGO_ARTICULO_LINEA`, `DESCRIPCION_ARTICULO_LINEA`, `ZONA`, `PR' +
        'ECIOVENTA_ARTICULO_LINEA`, `CANTIDAD_LINEA`, `SUM_TOTAL_LINEA`, ' +
        '`ODONTOLOGO`)'
      'VALUES'
      
        '  (:`SERIE_PRESUPUESTO_LINEA`, :`NRO_PRESUPUESTO_LINEA`, :`LINEA_LINEA`,' +
        ' :`CODIGO_ARTICULO_LINEA`, :`DESCRIPCION_ARTICULO_LINEA`, :`ZONA' +
        '`, :`PRECIOVENTA_ARTICULO_LINEA`, :`CANTIDAD_LINEA`, :`SUM_TOTAL' +
        '_LINEA`, :`ODONTOLOGO`)')
    SQLDelete.Strings = (
      'DELETE FROM `SUBOC_FACTURAS_LINEAS`'
      'WHERE'
      
        '  `SERIE_PRESUPUESTO_LINEA` = :`Old_SERIE_PRESUPUESTO_LINEA` AND `NRO_FA' +
        'CTURA_LINEA` = :`Old_NRO_PRESUPUESTO_LINEA` AND `LINEA_LINEA` = :`Ol' +
        'd_LINEA_LINEA`')
    SQLUpdate.Strings = (
      'UPDATE `SUBOC_FACTURAS_LINEAS`'
      'SET'
      
        '  `SERIE_PRESUPUESTO_LINEA` = :`SERIE_PRESUPUESTO_LINEA`, `NRO_PRESUPUESTO_L' +
        'INEA` = :`NRO_PRESUPUESTO_LINEA`, `LINEA_LINEA` = :`LINEA_LINEA`, `C' +
        'ODIGO_ARTICULO_LINEA` = :`CODIGO_ARTICULO_LINEA`, `DESCRIPCION_A' +
        'RTICULO_LINEA` = :`DESCRIPCION_ARTICULO_LINEA`, `ZONA` = :`ZONA`' +
        ', `PRECIOVENTA_ARTICULO_LINEA` = :`PRECIOVENTA_ARTICULO_LINEA`, ' +
        '`CANTIDAD_LINEA` = :`CANTIDAD_LINEA`, `SUM_TOTAL_LINEA` = :`SUM_' +
        'TOTAL_LINEA`, `ODONTOLOGO` = :`ODONTOLOGO`'
      'WHERE'
      
        '  `SERIE_PRESUPUESTO_LINEA` = :`Old_SERIE_PRESUPUESTO_LINEA` AND `NRO_FA' +
        'CTURA_LINEA` = :`Old_NRO_PRESUPUESTO_LINEA` AND `LINEA_LINEA` = :`Ol' +
        'd_LINEA_LINEA`')
    SQLLock.Strings = (
      'SELECT * FROM SUBOC_FACTURAS_LINEAS'
      'WHERE'
      
        '  `SERIE_PRESUPUESTO_LINEA` = :`Old_SERIE_PRESUPUESTO_LINEA` AND `NRO_FA' +
        'CTURA_LINEA` = :`Old_NRO_PRESUPUESTO_LINEA` AND `LINEA_LINEA` = :`Ol' +
        'd_LINEA_LINEA`'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT `SERIE_PRESUPUESTO_LINEA`, `NRO_PRESUPUESTO_LINEA`, `LINEA_LINEA`' +
        ', `CODIGO_ARTICULO_LINEA`, `DESCRIPCION_ARTICULO_LINEA`, `ZONA`,' +
        ' `PRECIOVENTA_ARTICULO_LINEA`, `CANTIDAD_LINEA`, `SUM_TOTAL_LINE' +
        'A`, `ODONTOLOGO` FROM `SUBOC_FACTURAS_LINEAS`'
      'WHERE'
      
        '  `SERIE_PRESUPUESTO_LINEA` = :`SERIE_PRESUPUESTO_LINEA` AND `NRO_FACTUR' +
        'A_LINEA` = :`NRO_PRESUPUESTO_LINEA` AND `LINEA_LINEA` = :`LINEA_LINE' +
        'A`')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM SUBOC_FACTURAS_LINEAS')
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT *'
      ' FROM SUBOC_PRESUPUESTOS_LINEAS'
      'WHERE NRO_PRESUPUESTO_LINEA = :NRO_PRESUPUESTO'
      '  AND SERIE_PRESUPUESTO_LINEA = :SERIE_PRESUPUESTO'
      'ORDER BY LINEA_LINEA;')
    MasterSource = dsFacPrint
    MasterFields = 'SERIE_PRESUPUESTO;NRO_PRESUPUESTO'
    DetailFields = 'SERIE_PRESUPUESTO_LINEA;NRO_PRESUPUESTO_LINEA'
    Left = 288
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NRO_PRESUPUESTO'
        ParamType = ptInput
        Value = 3
      end
      item
        DataType = ftString
        Name = 'SERIE_PRESUPUESTO'
        ParamType = ptInput
        Value = 'P19'
      end>
  end
  object unqrySeries: TUniQuery
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT DISTINCT SERIE_CONTADOR'
      'FROM SUBOC_CONTADORES'
      'WHERE TIPODOC_CONTADOR='#39'PR'#39
      'AND DEFAULT_CONTADOR = '#39'S'#39
      'ORDER BY DEFAULT_CONTADOR DESC')
    Left = 688
    Top = 144
  end
  object unqryCliDataFac: TUniQuery
    Connection = dmConn.conUni
    SQL.Strings = (
      'select * from suboc_clientes'
      'order by convert(CODIGO_CLIENTE, int) DESC')
    Left = 532
    Top = 8
  end
  object unqryArtDataLinFac: TUniQuery
    Connection = dmConn.conUni
    SQL.Strings = (
      'select * from suboc_articulos')
    Left = 626
    Top = 8
  end
  object unqryDocDataFac: TUniQuery
    Connection = dmConn.conUni
    SQL.Strings = (
      'select * from suboc_odontologos')
    Left = 724
    Top = 8
  end
  object unqryGetLinea: TUniQuery
    Connection = dmConn.conUni
    Left = 808
    Top = 8
  end
  object unqryLinFac: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO suboc_presupuestos_lineas'
      
        '  (SERIE_PRESUPUESTO_LINEA, NRO_PRESUPUESTO_LINEA, LINEA_LINEA, CODIGO_A' +
        'RTICULO_LINEA, DESCRIPCION_ARTICULO_LINEA, ZONA, PRECIOVENTA_ART' +
        'ICULO_LINEA, CANTIDAD_LINEA, SUM_TOTAL_LINEA, ODONTOLOGO)'
      'VALUES'
      
        '  (:SERIE_PRESUPUESTO_LINEA, :NRO_PRESUPUESTO_LINEA, :LINEA_LINEA, :CODI' +
        'GO_ARTICULO_LINEA, :DESCRIPCION_ARTICULO_LINEA, :ZONA, :PRECIOVE' +
        'NTA_ARTICULO_LINEA, :CANTIDAD_LINEA, :SUM_TOTAL_LINEA, :ODONTOLO' +
        'GO)')
    SQLDelete.Strings = (
      'DELETE FROM suboc_presupuestos_lineas'
      'WHERE'
      
        '  SERIE_PRESUPUESTO_LINEA = :Old_SERIE_PRESUPUESTO_LINEA AND NRO_PRESUPUESTO' +
        '_LINEA = :Old_NRO_PRESUPUESTO_LINEA AND LINEA_LINEA = :Old_LINEA_LIN' +
        'EA')
    SQLUpdate.Strings = (
      'UPDATE suboc_presupuestos_lineas'
      'SET'
      
        '  SERIE_PRESUPUESTO_LINEA = :SERIE_PRESUPUESTO_LINEA, NRO_PRESUPUESTO_LINEA ' +
        '= :NRO_PRESUPUESTO_LINEA, LINEA_LINEA = :LINEA_LINEA, CODIGO_ARTICUL' +
        'O_LINEA = :CODIGO_ARTICULO_LINEA, DESCRIPCION_ARTICULO_LINEA = :' +
        'DESCRIPCION_ARTICULO_LINEA, ZONA = :ZONA, PRECIOVENTA_ARTICULO_L' +
        'INEA = :PRECIOVENTA_ARTICULO_LINEA, CANTIDAD_LINEA = :CANTIDAD_L' +
        'INEA, SUM_TOTAL_LINEA = :SUM_TOTAL_LINEA, ODONTOLOGO = :ODONTOLO' +
        'GO'
      'WHERE'
      
        '  SERIE_PRESUPUESTO_LINEA = :Old_SERIE_PRESUPUESTO_LINEA AND NRO_PRESUPUESTO' +
        '_LINEA = :Old_NRO_PRESUPUESTO_LINEA AND LINEA_LINEA = :Old_LINEA_LIN' +
        'EA')
    SQLLock.Strings = (
      'SELECT * FROM suboc_presupuestos_lineas'
      'WHERE'
      
        '  SERIE_PRESUPUESTO_LINEA = :Old_SERIE_PRESUPUESTO_LINEA AND NRO_PRESUPUESTO' +
        '_LINEA = :Old_NRO_PRESUPUESTO_LINEA AND LINEA_LINEA = :Old_LINEA_LIN' +
        'EA'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT SERIE_PRESUPUESTO_LINEA, NRO_PRESUPUESTO_LINEA, LINEA_LINEA, CODI' +
        'GO_ARTICULO_LINEA, DESCRIPCION_ARTICULO_LINEA, ZONA, PRECIOVENTA' +
        '_ARTICULO_LINEA, CANTIDAD_LINEA, SUM_TOTAL_LINEA, ODONTOLOGO FRO' +
        'M suboc_presupuestos_lineas'
      'WHERE'
      
        '  SERIE_PRESUPUESTO_LINEA = :SERIE_PRESUPUESTO_LINEA AND NRO_PRESUPUESTO_LIN' +
        'EA = :NRO_PRESUPUESTO_LINEA AND LINEA_LINEA = :LINEA_LINEA')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_presupuestos_lineas')
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT *'
      'FROM SUBOC_PRESUPUESTOS_LINEAS'
      'where NRO_PRESUPUESTO_LINEA = :NRO_PRESUPUESTO'
      'AND SERIE_PRESUPUESTO_LINEA = :SERIE_PRESUPUESTO'
      'order by NRO_PRESUPUESTO_LINEA, SERIE_PRESUPUESTO_LINEA, LINEA_LINEA ASC')
    MasterSource = frmMtoPre.dsTablaG
    MasterFields = 'SERIE_PRESUPUESTO;NRO_PRESUPUESTO'
    DetailFields = 'SERIE_PRESUPUESTO_LINEA;NRO_PRESUPUESTO_LINEA'
    BeforeInsert = zqryLinFacBeforeInsert
    BeforePost = zqryLinFacBeforePost
    AfterPost = zqryLinFacAfterPost
    AfterDelete = zqryLinFacAfterDelete
    Left = 128
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NRO_PRESUPUESTO'
        ParamType = ptInput
        Value = 5
      end
      item
        DataType = ftString
        Name = 'SERIE_PRESUPUESTO'
        ParamType = ptInput
        Value = 'Z19'
      end>
  end
  object unstrdprcGetContadorFactura: TUniStoredProc
    StoredProcName = 'GET_NEXT_CONT_FACT_SERIE'
    SQL.Strings = (
      
        'CALL GET_NEXT_CONT_FACT_SERIE(:pserie, :pTipoDoc, @pcont); SELEC' +
        'T CAST(@pcont AS SIGNED) AS '#39'@pcont'#39)
    Connection = dmConn.conUni
    Left = 208
    Top = 288
    ParamData = <
      item
        DataType = ftString
        Name = 'pserie'
        ParamType = ptInput
        Size = 10
        Value = nil
      end
      item
        DataType = ftString
        Name = 'pTipoDoc'
        ParamType = ptInput
        Size = 2
        Value = nil
      end
      item
        DataType = ftInteger
        Name = 'pcont'
        ParamType = ptOutput
        Value = nil
      end>
    CommandStoredProcName = 'GET_NEXT_CONT_FACT_SERIE'
  end
  object unstrdprcGetContador: TUniStoredProc
    StoredProcName = 'GET_NEXT_CONT'
    SQL.Strings = (
      
        'CALL GET_NEXT_CONT(:pTipoDoc, @pcont); SELECT CAST(@pcont AS SIG' +
        'NED) AS '#39'@pcont'#39)
    Connection = dmConn.conUni
    Left = 360
    Top = 288
    ParamData = <
      item
        DataType = ftString
        Name = 'pTipoDoc'
        ParamType = ptInput
        Size = 2
        Value = nil
      end
      item
        DataType = ftInteger
        Name = 'pcont'
        ParamType = ptOutput
        Value = Null
      end>
    CommandStoredProcName = 'GET_NEXT_CONT'
    StoredProcIsQuery = True
  end
  object unstrdprcCrearCliente: TUniStoredProc
    StoredProcName = 'PRC_CREAR_ACTUALIZAR_CLIENTE'
    SQL.Strings = (
      
        'CALL PRC_CREAR_ACTUALIZAR_CLIENTE(:pCODIGO_CLIENTE, :pRAZONSOCIA' +
        'L_CLIENTE, :pNOMBRE, :pAPELLIDOS, :pNIF_CLIENTE, :pMOVIL_CLIENTE' +
        ', :pEMAIL_CLIENTE, :pDIRECCION1_CLIENTE, :pDIRECCION2_CLIENTE, :' +
        'pPOBLACION_CLIENTE, :pPROVINCIA_CLIENTE, :pCPOSTAL_CLIENTE, :pPA' +
        'IS_CLIENTE, :pTIPOID_INT_CLIENTE)')
    Connection = dmConn.conUni
    Left = 496
    Top = 288
    ParamData = <
      item
        DataType = ftInteger
        Name = 'pCODIGO_CLIENTE'
        ParamType = ptInput
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pRAZONSOCIAL_CLIENTE'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pNOMBRE'
        ParamType = ptInput
        Size = 100
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pAPELLIDOS'
        ParamType = ptInput
        Size = 100
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pNIF_CLIENTE'
        ParamType = ptInput
        Size = 50
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pMOVIL_CLIENTE'
        ParamType = ptInput
        Size = 40
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pEMAIL_CLIENTE'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pDIRECCION1_CLIENTE'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pDIRECCION2_CLIENTE'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pPOBLACION_CLIENTE'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pPROVINCIA_CLIENTE'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pCPOSTAL_CLIENTE'
        ParamType = ptInput
        Size = 15
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pPAIS_CLIENTE'
        ParamType = ptInput
        Size = 150
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pTIPOID_INT_CLIENTE'
        ParamType = ptInput
        Size = 150
        Value = nil
      end>
    CommandStoredProcName = 'PRC_CREAR_ACTUALIZAR_CLIENTE'
  end
  object unstrdprcGetDataArticulo: TUniStoredProc
    StoredProcName = 'PRC_GET_DATA_ARTICULO'
    SQL.Strings = (
      
        'CALL PRC_GET_DATA_ARTICULO(:pidcodarticulo, @pidnomarticulo, @pi' +
        'dprecioventa); SELECT @pidnomarticulo AS '#39'@pidnomarticulo'#39', @pid' +
        'precioventa AS '#39'@pidprecioventa'#39)
    Connection = dmConn.conUni
    Left = 624
    Top = 288
    ParamData = <
      item
        DataType = ftString
        Name = 'pidcodarticulo'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftString
        Name = 'pidnomarticulo'
        ParamType = ptOutput
        Size = 200
        Value = ''
      end
      item
        DataType = ftFloat
        Name = 'pidprecioventa'
        ParamType = ptOutput
        Value = 0.000000000000000000
      end>
    CommandStoredProcName = 'PRC_GET_DATA_ARTICULO'
  end
  object unstrdprcGetDataCliente: TUniStoredProc
    StoredProcName = 'PRC_GET_DATA_CLIENTE'
    SQL.Strings = (
      
        'CALL PRC_GET_DATA_CLIENTE(:pCODIGO_CLIENTE, @pRAZONSOCIAL_CLIENT' +
        'E, @pNOMBRE, @pAPELLIDOS, @pNIF_CLIENTE, @pMOVIL_CLIENTE, @pEMAI' +
        'L_CLIENTE, @pDIRECCION1_CLIENTE, @pDIRECCION2_CLIENTE, @pPOBLACI' +
        'ON_CLIENTE, @pPROVINCIA_CLIENTE, @pCPOSTAL_CLIENTE, @pPAIS_CLIEN' +
        'TE, @pTIPOID_INT_CLIENTE); SELECT @pRAZONSOCIAL_CLIENTE AS '#39'@pRA' +
        'ZONSOCIAL_CLIENTE'#39', @pNOMBRE AS '#39'@pNOMBRE'#39', @pAPELLIDOS AS '#39'@pAP' +
        'ELLIDOS'#39', @pNIF_CLIENTE AS '#39'@pNIF_CLIENTE'#39', @pMOVIL_CLIENTE AS '#39 +
        '@pMOVIL_CLIENTE'#39', @pEMAIL_CLIENTE AS '#39'@pEMAIL_CLIENTE'#39', @pDIRECC' +
        'ION1_CLIENTE AS '#39'@pDIRECCION1_CLIENTE'#39', @pDIRECCION2_CLIENTE AS ' +
        #39'@pDIRECCION2_CLIENTE'#39', @pPOBLACION_CLIENTE AS '#39'@pPOBLACION_CLIE' +
        'NTE'#39', @pPROVINCIA_CLIENTE AS '#39'@pPROVINCIA_CLIENTE'#39', @pCPOSTAL_CL' +
        'IENTE AS '#39'@pCPOSTAL_CLIENTE'#39', @pPAIS_CLIENTE AS '#39'@pPAIS_CLIENTE'#39 +
        ', @pTIPOID_INT_CLIENTE AS '#39'@pTIPOID_INT_CLIENTE'#39)
    Connection = dmConn.conUni
    Left = 760
    Top = 288
    ParamData = <
      item
        DataType = ftInteger
        Name = 'pCODIGO_CLIENTE'
        ParamType = ptInput
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pRAZONSOCIAL_CLIENTE'
        ParamType = ptOutput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pNOMBRE'
        ParamType = ptOutput
        Size = 100
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pAPELLIDOS'
        ParamType = ptOutput
        Size = 100
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pNIF_CLIENTE'
        ParamType = ptOutput
        Size = 50
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pMOVIL_CLIENTE'
        ParamType = ptOutput
        Size = 40
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pEMAIL_CLIENTE'
        ParamType = ptOutput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pDIRECCION1_CLIENTE'
        ParamType = ptOutput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pDIRECCION2_CLIENTE'
        ParamType = ptOutput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pPOBLACION_CLIENTE'
        ParamType = ptOutput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pPROVINCIA_CLIENTE'
        ParamType = ptOutput
        Size = 200
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pCPOSTAL_CLIENTE'
        ParamType = ptOutput
        Size = 15
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pPAIS_CLIENTE'
        ParamType = ptOutput
        Size = 150
        Value = nil
      end
      item
        DataType = ftWideString
        Name = 'pTIPOID_INT_CLIENTE'
        ParamType = ptOutput
        Size = 150
        Value = nil
      end>
    CommandStoredProcName = 'PRC_GET_DATA_CLIENTE'
  end
  object dsFormasPago: TDataSource
    DataSet = unqryFormaPago
    Left = 552
    Top = 88
  end
  object unqryFormaPago: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO suboc_historia'
      
        '  (ID, CODIGO_ARTICULO, DESCRIPCION_ARTICULO, CODIGO_CLIENTE, PR' +
        'ECIOVENTA_ARTICULO, FECHA, ZONA, DESCRIPCION_HISTORIA, NRO_FACTU' +
        'RA, LINEA_LINEA, ODONTOLOGO, SERIE_PRESUPUESTO)'
      'VALUES'
      
        '  (:ID, :CODIGO_ARTICULO, :DESCRIPCION_ARTICULO, :CODIGO_CLIENTE' +
        ', :PRECIOVENTA_ARTICULO, :FECHA, :ZONA, :DESCRIPCION_HISTORIA, :' +
        'NRO_PRESUPUESTO, :LINEA_LINEA, :ODONTOLOGO, :SERIE_PRESUPUESTO)')
    SQLDelete.Strings = (
      'DELETE FROM suboc_historia'
      'WHERE'
      '  ID = :Old_ID')
    SQLUpdate.Strings = (
      'UPDATE suboc_historia'
      'SET'
      
        '  ID = :ID, CODIGO_ARTICULO = :CODIGO_ARTICULO, DESCRIPCION_ARTI' +
        'CULO = :DESCRIPCION_ARTICULO, CODIGO_CLIENTE = :CODIGO_CLIENTE, ' +
        'PRECIOVENTA_ARTICULO = :PRECIOVENTA_ARTICULO, FECHA = :FECHA, ZO' +
        'NA = :ZONA, DESCRIPCION_HISTORIA = :DESCRIPCION_HISTORIA, NRO_FA' +
        'CTURA = :NRO_PRESUPUESTO, LINEA_LINEA = :LINEA_LINEA, ODONTOLOGO = :' +
        'ODONTOLOGO, SERIE_PRESUPUESTO = :SERIE_PRESUPUESTO'
      'WHERE'
      '  ID = :Old_ID')
    SQLLock.Strings = (
      'SELECT * FROM suboc_historia'
      'WHERE'
      '  ID = :Old_ID'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT ID, CODIGO_ARTICULO, DESCRIPCION_ARTICULO, CODIGO_CLIENTE' +
        ', PRECIOVENTA_ARTICULO, FECHA, ZONA, DESCRIPCION_HISTORIA, NRO_F' +
        'ACTURA, LINEA_LINEA, ODONTOLOGO, SERIE_PRESUPUESTO FROM suboc_histor' +
        'ia'
      'WHERE'
      '  ID = :ID')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_historia')
    Connection = dmConn.conUni
    SQL.Strings = (
      'select * from suboc_formapago'
      '')
    ReadOnly = True
    Left = 552
    Top = 144
  end
  object dsRecibos: TDataSource
    DataSet = unqryRecibos
    Left = 632
    Top = 88
  end
  object unqryRecibos: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO `suboc_recibos`'
      
        '  (`NRO_PRESUPUESTO`, `SERIE_PRESUPUESTO`, `NRO_PLAZO_RECIBO`, `FORMA_PA' +
        'GO_ORIGEN`, `FORMA_PAGO_DESCRIPCION_ORIGEN`, `EUROS_RECIBO`, `ES' +
        'TADO_RECIBO`, `FECHA_EXPEDICION`, `FECHA_VENCIMIENTO`, `IBAN`, `' +
        'FECHA_PAGO`, `LOCALIDAD_EXPEDICION`, `CODIGO_CLIENTE`, `RAZONSOC' +
        'IAL_CLIENTE`, `DIRECCION1_CLIENTE`, `POBLACION_CLIENTE`, `PROVIN' +
        'CIA_CLIENTE`, `CPOSTAL_CLIENTE`, `IMPORTE_LETRA`, `TRATAMIENTOS`' +
        ')'
      'VALUES'
      
        '  (:`NRO_PRESUPUESTO`, :`SERIE_PRESUPUESTO`, :`NRO_PLAZO_RECIBO`, :`FORM' +
        'A_PAGO_ORIGEN`, :`FORMA_PAGO_DESCRIPCION_ORIGEN`, :`EUROS_RECIBO' +
        '`, :`ESTADO_RECIBO`, :`FECHA_EXPEDICION`, :`FECHA_VENCIMIENTO`, ' +
        ':`IBAN`, :`FECHA_PAGO`, :`LOCALIDAD_EXPEDICION`, :`CODIGO_CLIENT' +
        'E`, :`RAZONSOCIAL_CLIENTE`, :`DIRECCION1_CLIENTE`, :`POBLACION_C' +
        'LIENTE`, :`PROVINCIA_CLIENTE`, :`CPOSTAL_CLIENTE`, :`IMPORTE_LET' +
        'RA`, :`TRATAMIENTOS`)')
    SQLDelete.Strings = (
      'DELETE FROM `suboc_recibos`'
      'WHERE'
      
        '  `NRO_PRESUPUESTO` = :`Old_NRO_PRESUPUESTO` AND `SERIE_PRESUPUESTO` = :`Old' +
        '_SERIE_PRESUPUESTO` AND `NRO_PLAZO_RECIBO` = :`Old_NRO_PLAZO_RECIBO`')
    SQLUpdate.Strings = (
      'UPDATE `suboc_recibos`'
      'SET'
      
        '  `NRO_PRESUPUESTO` = :`NRO_PRESUPUESTO`, `SERIE_PRESUPUESTO` = :`SERIE_FACT' +
        'URA`, `NRO_PLAZO_RECIBO` = :`NRO_PLAZO_RECIBO`, `FORMA_PAGO_ORIG' +
        'EN` = :`FORMA_PAGO_ORIGEN`, `FORMA_PAGO_DESCRIPCION_ORIGEN` = :`' +
        'FORMA_PAGO_DESCRIPCION_ORIGEN`, `EUROS_RECIBO` = :`EUROS_RECIBO`' +
        ', `ESTADO_RECIBO` = :`ESTADO_RECIBO`, `FECHA_EXPEDICION` = :`FEC' +
        'HA_EXPEDICION`, `FECHA_VENCIMIENTO` = :`FECHA_VENCIMIENTO`, `IBA' +
        'N` = :`IBAN`, `FECHA_PAGO` = :`FECHA_PAGO`, `LOCALIDAD_EXPEDICIO' +
        'N` = :`LOCALIDAD_EXPEDICION`, `CODIGO_CLIENTE` = :`CODIGO_CLIENT' +
        'E`, `RAZONSOCIAL_CLIENTE` = :`RAZONSOCIAL_CLIENTE`, `DIRECCION1_' +
        'CLIENTE` = :`DIRECCION1_CLIENTE`, `POBLACION_CLIENTE` = :`POBLAC' +
        'ION_CLIENTE`, `PROVINCIA_CLIENTE` = :`PROVINCIA_CLIENTE`, `CPOST' +
        'AL_CLIENTE` = :`CPOSTAL_CLIENTE`, `IMPORTE_LETRA` = :`IMPORTE_LE' +
        'TRA`, `TRATAMIENTOS` = :`TRATAMIENTOS`'
      'WHERE'
      
        '  `NRO_PRESUPUESTO` = :`Old_NRO_PRESUPUESTO` AND `SERIE_PRESUPUESTO` = :`Old' +
        '_SERIE_PRESUPUESTO` AND `NRO_PLAZO_RECIBO` = :`Old_NRO_PLAZO_RECIBO`')
    SQLLock.Strings = (
      'SELECT * FROM suboc_recibos'
      'WHERE'
      
        '  `NRO_PRESUPUESTO` = :`Old_NRO_PRESUPUESTO` AND `SERIE_PRESUPUESTO` = :`Old' +
        '_SERIE_PRESUPUESTO` AND `NRO_PLAZO_RECIBO` = :`Old_NRO_PLAZO_RECIBO`'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT `NRO_PRESUPUESTO`, `SERIE_PRESUPUESTO`, `NRO_PLAZO_RECIBO`, `FORM' +
        'A_PAGO_ORIGEN`, `FORMA_PAGO_DESCRIPCION_ORIGEN`, `EUROS_RECIBO`,' +
        ' `ESTADO_RECIBO`, `FECHA_EXPEDICION`, `FECHA_VENCIMIENTO`, `IBAN' +
        '`, `FECHA_PAGO`, `LOCALIDAD_EXPEDICION`, `CODIGO_CLIENTE`, `RAZO' +
        'NSOCIAL_CLIENTE`, `DIRECCION1_CLIENTE`, `POBLACION_CLIENTE`, `PR' +
        'OVINCIA_CLIENTE`, `CPOSTAL_CLIENTE`, `IMPORTE_LETRA`, `TRATAMIEN' +
        'TOS` FROM `suboc_recibos`'
      'WHERE'
      
        '  `NRO_PRESUPUESTO` = :`P_1_NRO_PRESUPUESTO` AND `SERIE_PRESUPUESTO` = :`P_1' +
        '_SERIE_PRESUPUESTO` AND `NRO_PLAZO_RECIBO` = :`NRO_PLAZO_RECIBO`')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_recibos')
    Connection = dmConn.conUni
    SQL.Strings = (
      'select * from suboc_recibos'
      '')
    MasterSource = frmMtoPre.dsTablaG
    MasterFields = 'NRO_PRESUPUESTO;SERIE_PRESUPUESTO'
    DetailFields = 'NRO_PRESUPUESTO;SERIE_PRESUPUESTO'
    Left = 632
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NRO_PRESUPUESTO'
        ParamType = ptInput
        Value = 5
      end
      item
        DataType = ftString
        Name = 'SERIE_PRESUPUESTO'
        ParamType = ptInput
        Value = 'Z19'
      end>
  end
  object dsRecibosPrint: TDataSource
    DataSet = unqryRecibosPrint
    Left = 368
    Top = 88
  end
  object fxdsRecibos: TfrxDBDataset
    Description = 'Recibos'
    UserName = 'Facturas'
    CloseDataSource = False
    FieldAliases.Strings = (
      'NRO_PRESUPUESTO=NRO_PRESUPUESTO'
      'SERIE_PRESUPUESTO=SERIE_PRESUPUESTO'
      'NRO_PLAZO_RECIBO=NRO_PLAZO_RECIBO'
      'FORMA_PAGO_ORIGEN=FORMA_PAGO_ORIGEN'
      'FORMA_PAGO_DESCRIPCION_ORIGEN=FORMA_PAGO_DESCRIPCION_ORIGEN'
      'EUROS_RECIBO=EUROS_RECIBO'
      'ESTADO_RECIBO=ESTADO_RECIBO'
      'FECHA_EXPEDICION=FECHA_EXPEDICION'
      'FECHA_VENCIMIENTO=FECHA_VENCIMIENTO'
      'IBAN=IBAN'
      'FECHA_PAGO=FECHA_PAGO'
      'LOCALIDAD_EXPEDICION=LOCALIDAD_EXPEDICION'
      'CODIGO_CLIENTE=CODIGO_CLIENTE'
      'RAZONSOCIAL_CLIENTE=RAZONSOCIAL_CLIENTE'
      'DIRECCION1_CLIENTE=DIRECCION1_CLIENTE'
      'POBLACION_CLIENTE=POBLACION_CLIENTE'
      'PROVINCIA_CLIENTE=PROVINCIA_CLIENTE'
      'CPOSTAL_CLIENTE=CPOSTAL_CLIENTE'
      'IMPORTE_LETRA=IMPORTE_LETRA'
      'TRATAMIENTOS=TRATAMIENTOS')
    DataSource = dsRecibosPrint
    BCDToCurrency = False
    DataSetOptions = []
    Left = 368
    Top = 24
  end
  object unqryRecibosPrint: TUniQuery
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT * '
      'from SUBOC_recibos'
      '')
    DMLRefresh = True
    Left = 368
    Top = 144
  end
  object unstrdprcGetRecibos: TUniStoredProc
    StoredProcName = 'PRC_CREAR_RECIBOS_PRESUPUESTO'
    SQL.Strings = (
      
        'CALL PRC_CREAR_RECIBOS_PRESUPUESTO(:pSERIE_PRESUPUESTO, :pNRO_FACTUR' +
        'A)')
    Connection = dmConn.conUni
    Left = 888
    Top = 288
    ParamData = <
      item
        DataType = ftString
        Name = 'pSERIE_PRESUPUESTO'
        ParamType = ptInput
        Size = 8
        Value = nil
      end
      item
        DataType = ftInteger
        Name = 'pNRO_PRESUPUESTO'
        ParamType = ptInput
        Value = nil
      end>
    CommandStoredProcName = 'PRC_CREAR_RECIBOS_PRESUPUESTO'
  end
  object unstrdprcCalcularFactura: TUniStoredProc
    StoredProcName = 'PRC_CALCULAR_PRESUPUESTO'
    SQL.Strings = (
      'CALL PRC_CALCULAR_PRESUPUESTO(:pidseriefactura, :pidnumfactura)')
    Connection = dmConn.conUni
    Left = 64
    Top = 288
    ParamData = <
      item
        DataType = ftString
        Name = 'pidseriefactura'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftString
        Name = 'pidnumfactura'
        ParamType = ptInput
        Size = 200
        Value = nil
      end>
    CommandStoredProcName = 'PRC_CALCULAR_PRESUPUESTO'
  end
  object unqryDibujos: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO suboc_dibujos_presupuestos'
      '  (NRO_PRESUPUESTO, SERIE_PRESUPUESTO, DIBUJO_PRESUPUESTO)'
      'VALUES'
      '  (:NRO_PRESUPUESTO, :SERIE_PRESUPUESTO, :DIBUJO_PRESUPUESTO)')
    SQLDelete.Strings = (
      'DELETE FROM suboc_dibujos_presupuestos'
      'WHERE'
      
        '  NRO_PRESUPUESTO = :Old_NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :Old_SERIE_' +
        'FACTURA')
    SQLUpdate.Strings = (
      'UPDATE suboc_dibujos_presupuestos'
      'SET'
      
        '  NRO_PRESUPUESTO = :NRO_PRESUPUESTO, SERIE_PRESUPUESTO = :SERIE_PRESUPUESTO, DI' +
        'BUJO_FACTURA = :DIBUJO_PRESUPUESTO'
      'WHERE'
      
        '  NRO_PRESUPUESTO = :Old_NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :Old_SERIE_' +
        'FACTURA')
    SQLLock.Strings = (
      'SELECT * FROM suboc_dibujos_presupuestos'
      'WHERE'
      
        '  NRO_PRESUPUESTO = :Old_NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :Old_SERIE_' +
        'FACTURA'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT NRO_PRESUPUESTO, SERIE_PRESUPUESTO, DIBUJO_PRESUPUESTO FROM suboc_dib' +
        'ujos_presupuestos'
      'WHERE'
      '  NRO_PRESUPUESTO = :NRO_PRESUPUESTO AND SERIE_PRESUPUESTO = :SERIE_PRESUPUESTO')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_dibujos_presupuestos')
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT * FROM suboc_dibujos_presupuestos'
      'WHERE NRO_PRESUPUESTO = :NRO_PRESUPUESTO'
      'AND SERIE_PRESUPUESTO = :SERIE_PRESUPUESTO')
    MasterSource = frmMtoPre.dsTablaG
    MasterFields = 'NRO_PRESUPUESTO;SERIE_PRESUPUESTO'
    DetailFields = 'NRO_PRESUPUESTO;SERIE_PRESUPUESTO'
    Left = 888
    Top = 144
    ParamData = <
      item
        DataType = ftInteger
        Name = 'NRO_PRESUPUESTO'
        ParamType = ptInput
        Value = 4
      end
      item
        DataType = ftWideString
        Name = 'SERIE_PRESUPUESTO'
        ParamType = ptInput
        Value = 'P25'
      end>
  end
  object dsSeriesFac: TDataSource
    DataSet = unqrySeriesFac
    Left = 768
    Top = 88
  end
  object unqrySeriesFac: TUniQuery
    Connection = dmConn.conUni
    SQL.Strings = (
      'SELECT DISTINCT SERIE_CONTADOR'
      'FROM SUBOC_CONTADORES'
      'WHERE TIPODOC_CONTADOR='#39'FC'#39
      'ORDER BY DEFAULT_CONTADOR DESC')
    Left = 768
    Top = 144
  end
  object unstrdprcCrearFactura: TUniStoredProc
    StoredProcName = 'PRC_CREAR_FACTURA_PRESUPUESTO'
    SQL.Strings = (
      
        'CALL PRC_CREAR_FACTURA_PRESUPUESTO(:pidseriepresupuesto, :pidnum' +
        'presupuesto, :pidseriefactura, :pfechafactura, :pidnumfactura, @' +
        'pidresulnumfac); SELECT @pidresulnumfac AS '#39'@pidresulnumfac'#39)
    Connection = dmConn.conUni
    Left = 1016
    Top = 288
    ParamData = <
      item
        DataType = ftString
        Name = 'pidseriepresupuesto'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftString
        Name = 'pidnumpresupuesto'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftString
        Name = 'pidseriefactura'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftDate
        Name = 'pfechafactura'
        ParamType = ptInput
        Value = nil
      end
      item
        DataType = ftString
        Name = 'pidnumfactura'
        ParamType = ptInput
        Size = 200
        Value = nil
      end
      item
        DataType = ftString
        Name = 'pidresulnumfac'
        ParamType = ptOutput
        Size = 200
        Value = nil
      end>
    CommandStoredProcName = 'PRC_CREAR_FACTURA_PRESUPUESTO'
  end
  object dsDibujos: TDataSource
    DataSet = unqryDibujos
    Left = 880
    Top = 96
  end
  object dsPaises: TDataSource
    DataSet = unqryPaisesCli
    Left = 984
    Top = 88
  end
  object unqryPaisesCli: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO suboc_paises'
      
        '  (COD_PAIS, COD_PAIS_ALPHA3, COD_PAIS_ALPHA2, NOMBRE_SPA_PAIS, ' +
        'NOMBRE_ENG_PAIS, ORDEN_PAIS)'
      'VALUES'
      
        '  (:COD_PAIS, :COD_PAIS_ALPHA3, :COD_PAIS_ALPHA2, :NOMBRE_SPA_PA' +
        'IS, :NOMBRE_ENG_PAIS, :ORDEN_PAIS)')
    SQLDelete.Strings = (
      'DELETE FROM suboc_paises'
      'WHERE'
      '  COD_PAIS = :Old_COD_PAIS')
    SQLUpdate.Strings = (
      'UPDATE suboc_paises'
      'SET'
      
        '  COD_PAIS = :COD_PAIS, COD_PAIS_ALPHA3 = :COD_PAIS_ALPHA3, COD_' +
        'PAIS_ALPHA2 = :COD_PAIS_ALPHA2, NOMBRE_SPA_PAIS = :NOMBRE_SPA_PA' +
        'IS, NOMBRE_ENG_PAIS = :NOMBRE_ENG_PAIS, ORDEN_PAIS = :ORDEN_PAIS'
      'WHERE'
      '  COD_PAIS = :Old_COD_PAIS')
    SQLLock.Strings = (
      
        'SELECT COD_PAIS, COD_PAIS_ALPHA3, COD_PAIS_ALPHA2, NOMBRE_SPA_PA' +
        'IS, NOMBRE_ENG_PAIS, ORDEN_PAIS FROM suboc_paises'
      'WHERE'
      '  COD_PAIS = :Old_COD_PAIS'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT COD_PAIS, COD_PAIS_ALPHA3, COD_PAIS_ALPHA2, NOMBRE_SPA_PA' +
        'IS, NOMBRE_ENG_PAIS, ORDEN_PAIS FROM suboc_paises'
      'WHERE'
      '  COD_PAIS = :COD_PAIS')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_paises')
    Connection = dmConn.conUni
    SQL.Strings = (
      'select * from suboc_paises'
      '')
    MasterSource = frmMtoPre.dsTablaG
    MasterFields = 'PAIS_CLIENTE_PRESUPUESTO'
    DetailFields = 'COD_PAIS_ALPHA2'
    ReadOnly = True
    Left = 984
    Top = 144
    ParamData = <
      item
        DataType = ftWideString
        Name = 'PAIS_CLIENTE_PRESUPUESTO'
        ParamType = ptInput
        Value = 'ZA'
      end>
  end
  object unqryPaises: TUniQuery
    SQLInsert.Strings = (
      'INSERT INTO suboc_paises'
      
        '  (COD_PAIS, COD_PAIS_ALPHA3, COD_PAIS_ALPHA2, NOMBRE_SPA_PAIS, ' +
        'NOMBRE_ENG_PAIS, ESMIEMBRO_UE_PAIS, ORDEN_PAIS)'
      'VALUES'
      
        '  (:COD_PAIS, :COD_PAIS_ALPHA3, :COD_PAIS_ALPHA2, :NOMBRE_SPA_PA' +
        'IS, :NOMBRE_ENG_PAIS, :ESMIEMBRO_UE_PAIS, :ORDEN_PAIS)')
    SQLDelete.Strings = (
      'DELETE FROM suboc_paises'
      'WHERE'
      '  COD_PAIS = :Old_COD_PAIS')
    SQLUpdate.Strings = (
      'UPDATE suboc_paises'
      'SET'
      
        '  COD_PAIS = :COD_PAIS, COD_PAIS_ALPHA3 = :COD_PAIS_ALPHA3, COD_' +
        'PAIS_ALPHA2 = :COD_PAIS_ALPHA2, NOMBRE_SPA_PAIS = :NOMBRE_SPA_PA' +
        'IS, NOMBRE_ENG_PAIS = :NOMBRE_ENG_PAIS, ESMIEMBRO_UE_PAIS = :ESM' +
        'IEMBRO_UE_PAIS, ORDEN_PAIS = :ORDEN_PAIS'
      'WHERE'
      '  COD_PAIS = :Old_COD_PAIS')
    SQLLock.Strings = (
      
        'SELECT COD_PAIS, COD_PAIS_ALPHA3, COD_PAIS_ALPHA2, NOMBRE_SPA_PA' +
        'IS, NOMBRE_ENG_PAIS, ESMIEMBRO_UE_PAIS, ORDEN_PAIS FROM suboc_pa' +
        'ises'
      'WHERE'
      '  COD_PAIS = :Old_COD_PAIS'
      'FOR UPDATE')
    SQLRefresh.Strings = (
      
        'SELECT COD_PAIS, COD_PAIS_ALPHA3, COD_PAIS_ALPHA2, NOMBRE_SPA_PA' +
        'IS, NOMBRE_ENG_PAIS, ESMIEMBRO_UE_PAIS, ORDEN_PAIS FROM suboc_pa' +
        'ises'
      'WHERE'
      '  COD_PAIS = :COD_PAIS')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM suboc_paises')
    Connection = dmConn.conUni
    SQL.Strings = (
      'select * from suboc_paises'
      '')
    ReadOnly = True
    Left = 984
    Top = 208
  end
end
