...
uses
  ...
  cxDBNavigator, dxNumericWheelPicker, cxVariants,
  cxDBLabel, dxGDIPlusClasses, Vcl.ToolWin, System.UITypes,
  JvExControls, JvgImage, cxRadioGroup, dxSkinBasic,
...
type
  ...
  TfrmMtoPre = class(TfrmMtoGen)
    ...
    btnColorAzul: TcxButton;    // NUEVO bot�n para l�neas rectas
    pnl7: TPanel;
...
//    procedure btnRectificarClick(Sender: TObject);
    ...
    procedure btnColorAzulClick(Sender: TObject);
                                              // NUEVO procedimiento para l�neas
  ...
  private
    ...
    FBackBuffer: TBitmap;
    // NUEVAS variables para l�neas rectas
    ...
    procedure DrawLineOnBitmap(X1, Y1, X2, Y2: Integer);
                                            // NUEVA funci�n para dibujar l�neas
...
begin
  ...
  btnDibujar.Hint := 'Dibujo libre' + #13#10 +
                     'Permite dibujar l�neas continuas arrastrando el mouse';
...
begin
  ...
  FGhostX := -1;
  FGhostY := -1;
  // Variables para l�neas rectas
  FIsDrawingLine := False;
  FLineStartX := -1;
  FLineStartY := -1;
  FLineEndX := -1;
  FLineEndY := -1;
  // NUEVAS variables para formas geom�tricas
...
begin
  ...
  // Resetear variables seg�n el modo
  if FDrawMode <> dmLine then
  begin
    FIsDrawingLine := False;
    FLineStartX := -1;
...
begin
  ...
  // Habilitar bot�n guardar si hay cambios
...
begin
  ...
  FModified := True;
  UpdateButtonStates;
  //ShowMessage('Gr�fico reseteado a la imagen por defecto.');
end;
...
//      end;
//
//      dmLine:  // NUEVO: Manejo de l�neas rectas
//      begin
...
begin
  ...
  if (FDrawMode = dmErase) then
  begin
    // Actualizar posici�n del fantasma solo si cambi� significativamente
