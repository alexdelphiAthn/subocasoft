...
uses
  ...
  System.Variants, Vcl.Imaging.pngimage,
  IdSSLOpenSSL, Uni,
...
begin
  case CancelResponse.ErrorCode of
    ...
    3019: // Invoice not found in queue
      begin
        sMensajeError := 'No se encontr� la factura en la cola de Verifactu';
        sMensajeLog := 'Factura no encontrada en cola para cancelaci�n';
      end;
    2000..2999: // Errores de licencia/configuraci�n
      begin
        sMensajeError := 'Error de configuraci�n: ' +
                                                    CancelResponse.ErrorMessage;
        sMensajeLog := 'Error de configuraci�n en cancelaci�n';
      end;
    else
      begin
        sMensajeError := Format('Error desconocido (%d): %s',
                               ...
                               CancelResponse.ErrorMessage]);
        sMensajeLog := 'Error no catalogado en cancelaci�n';
      end;
  end;
...
begin
  Result := (FConfig.URL <> '') and
            ...
            (FConfig.Password <> '');
  if not Result then
    LogError('Configuraci�n de Verifactu incompleta');
end;
  ...
  begin
    try
      ...
      begin
        ...
        begin
          Result.ErrorMessage := Format('Error despu�s de %d reintentos: %s',
...
begin
  ...
  begin
    try
      ...
      // AUTENTICACI�N - La cabecera exacta que funciona en Insomnia
...
//          qryConsolidacion.Connection.Rollback;
        LogError('Error guardando consolidaci�n: ' + E.Message);
        raise;
      end;
    end;
  finally
...
begin
  ...
  if not ValidarFactura(Serie, Numero) then
    raise Exception.Create('Datos de factura no v�lidos para Verifactu');
  // Enviar a Verifactu
...
begin
  ...
  try
    ...
    RequestBody := IntToStr(AQueueId); // Solo el n�mero
...
//    begin
//      LogError('Error al cargar configuraci�n para consulta de estado');
//      Exit;
//    end;
...
//    if URL = '' then
//    begin
//      LogError('URL de consulta de estado est� vac�a');
//      Exit;
//    end;

    // Enviar petici�n GET usando REST
...
begin
  ...
  try
    ...
    qryConsolidacion.ReadOnly := False;
    qryConsolidacion.Open;
    // Si no existe, crear registro inicial con la petici�n
    if qryConsolidacion.RecordCount = 0 then
    begin
      IDConsolidacion := inLibData.GetNextCont('CS');
      qryConsolidacion.Insert;
