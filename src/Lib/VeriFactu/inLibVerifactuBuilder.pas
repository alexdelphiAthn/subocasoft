...
type
  ...
  // Record para configuraci�n de IVA
  TVatConfig = record
...
begin
  ...
  if IsSpanish then
  begin
    // Para clientes espa�oles usar irsId
...
begin
  ...
  begin
    ...
    if ((sTipoId = 'ID') or (sTipoId = '04')) then
      ...
      IdType := id06; // Por defecto: Documento oficial de identificaci�n expedido por el pa�s o territorio de residencia
...
begin
  ...
  VatConfigs[0] := TVatConfig.ForStandardVAT(21); // IVA est�ndar 21%
...
begin
  ...
  try
    // Replicar comportamiento original: IVA 0%, operaci�n E1, clave 01
    SetLength(VatConfigs, 1);
